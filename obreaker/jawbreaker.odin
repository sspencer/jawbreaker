...
main :: proc() {
    ...
    screen_size := BLOCK_SIZE * NUM_BLOCKS + (DISPLAY_PADDING + BOARD_PADDING) * 2
    ...
    for !rl.WindowShouldClose() {
        ...
        selection := get_connected_pieces(mouse_pos)
        made_move := false
        ...
        if made_move == false && len(selection) > 1 {
            draw_game_piece_highlights(selection)
        }
